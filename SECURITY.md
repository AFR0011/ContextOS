...
The repository's Stage 7 local assurance framework is defined in `docs/stage7-audit-plan.md` and `audits/stage7-controls.json`. Stage 8 adds deployment, hosted-preview, backup/restore, rollback, and operational evidence. Historical Stage 9 added lifecycle-specific static controls and browser tests for logout/device cleanup, multi-user selection, account deletion, and the pre-C8 recoverable-tombstone model. Historical Stage 10 final acceptance is complete for the portfolio-stage local-first boundary and added functional production-offline Search/history plus an automated public-claims audit. C8 later removed the tombstone persistence model; current C8-C10 assurance validates the canonical upsert-only product while preserving the older evidence as provenance. Current product positioning is self-hostable application software.
