...
async function countWorkspaceRows(userId: string) {
  const [domains, projects, tasks, captures, notes, deadlines, reviews, scratchpads, preferences] = await Promise.all([
  ...
  return { domains, projects, tasks, captures, notes, deadlines, reviews, scratchpads, preferences };
}
