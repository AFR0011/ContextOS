    ...
    <div className="cos-page" data-testid="area-detail">
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => setEditingArea((value) => !value)}
              aria-expanded={editingArea}
              aria-controls="area-details-editor"
              className="cos-btn cos-btn-secondary px-3 py-2 text-sm"
            >
              {editingArea ? <Check className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
              {editingArea ? "Done" : "Edit details"}
            </button>
      ...
      {editingArea ? (
        <Section title="Area details">
          <div id="area-details-editor" className="cos-surface p-4">
      ...
      <Section
        ...
        className={editingArea ? "mt-8" : ""}
        action={
          currentArea.state === "active" ? (
            <button
              type="button"
              onClick={() => setShowProjectComposer((value) => !value)}
              aria-expanded={showProjectComposer}
              aria-controls="area-project-composer"
              className="cos-btn cos-btn-secondary px-3 py-1.5 text-xs"
            >
              <Plus className="h-3.5 w-3.5" /> {showProjectComposer ? "Cancel" : "New project"}
            </button>
          ) : null
        }
      >
        {activeProjects.length ? (
          ...
          <EmptyState variant="compact" title="No active projects" />
        )}

        {currentArea.state === "active" && showProjectComposer ? (
          <div id="area-project-composer" className="cos-surface mt-3 grid gap-2 p-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_auto]">
            <input autoFocus value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="New Project name" aria-label="Project name" className="cos-input px-3 py-2 text-sm" />
        ...
        action={
          currentArea.state === "active" ? (
            <button
              ...
              aria-controls="area-task-composer"
              className="cos-btn cos-btn-secondary px-3 py-1.5 text-xs"
            >
      ...
      >
        {openTasks.length || doneTasks.length ? (
          ...
          <EmptyState variant="compact" title="No direct tasks" />
        )}

        {currentArea.state === "active" && showTaskComposer ? (
          <div id="area-task-composer" className="cos-surface mt-3 grid gap-2 p-3 lg:grid-cols-[minmax(0,1fr)_10rem_8rem_auto]">
            <input autoFocus value={taskTitle} onChange={(event) => setTaskTitle(event.target.value)} onKeyDown={(event) => event.key === "Enter" && !event.nativeEvent.isComposing && createTask()} placeholder="Add a direct task..." aria-label="Task title" className="cos-input px-3 py-2 text-sm" />
        ...
        action={
          currentArea.state === "active" ? (
            <button
              ...
              aria-controls="area-date-composer"
              className="cos-btn cos-btn-secondary px-3 py-1.5 text-xs"
            >
      ...
      >
        <div data-testid="area-dates">
          {directDates.length ? (
            ...
            <EmptyState variant="compact" title="No direct dates" />
          )}

          {currentArea.state === "active" && showDateComposer ? (
            <div id="area-date-composer" className="cos-surface mt-3 grid gap-2 p-3 md:grid-cols-2 xl:grid-cols-[7rem_minmax(0,1fr)_10rem_8rem_8rem_auto]">
              <select
                value={dateKind}
                onChange={(event) => {
              ...
              <input autoFocus value={dateTitle} onChange={(event) => setDateTitle(event.target.value)} placeholder="Add a direct Date..." aria-label="Date title" className="cos-input px-3 py-2 text-sm" />
